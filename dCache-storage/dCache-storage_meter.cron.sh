...
# $Id: dCache-storage_meter.cron.sh,v 1.4 2008-02-28 21:25:06 greenc Exp $
...
NCMeter=`ps -ef | grep dcacheStorageMeter.py | grep -v grep | wc -l`
if [ ${NCMeter} -ne 0 ]; then
  ${Logger} "There is a dcacheStorageMeter.py running already"
  exit 1
fi
